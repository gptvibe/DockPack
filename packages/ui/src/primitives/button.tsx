...
const variantClasses: Record<ButtonVariant, string> = {
  primary:
    "bg-primary text-primary-foreground shadow-glow hover:-translate-y-0.5 hover:bg-primary/92 hover:shadow-panel",
  secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/80",
  outline:
    "border border-border/70 bg-background/72 text-foreground hover:border-primary/24 hover:bg-accent hover:text-accent-foreground",
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, size = "md", type = "button", variant = "primary", ...props }, ref) => {
    return <button ref={ref} className={buttonVariants({ className, size, variant })} type={type} {...props} />;
  },
);
